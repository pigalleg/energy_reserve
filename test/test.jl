...
function test1(input_location = DEFAULT_LOCATION, reference_location = "./test/reference.csv")
    reference =  CSV.read(reference_location, DataFrame)
    
    gen_df, loads_multi_df, gen_variable_multi_df, storage_df, random_loads_multi_df = generate_input_data(n, input_location)
